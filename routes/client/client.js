...
router.get("/:id", async (req, res) => {
	try {
		res.json({
			...(await clientDB.getUserById(req.params.id)),
			password: null,
		});
	} catch (error) {
...
router.get("/:id/sessions", async (req, res) => {
	try {
		...
		if (clientSessions.length < 1)
			throw new httpError(
				404,
				"No coaching session records found for that user."
			);
...
router.get("/:id/sessions/:sessionID", async (req, res) => {
	try {
		...
		if (clientSession.length < 1)
			throw new httpError(
				404,
				"No coaching session records found for that session ID."
			);
